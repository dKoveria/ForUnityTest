...
Shader "Unlit/UnlitFresnelFromTexLightmapsCutoutShader"
{
	Properties
	{
	...
	CGINCLUDE
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				half3 fre = pow(1.0-max(0,dot(normalDirection, viewDirection)),0.218*_Fresnel)*unity_FogColor.rgb*_Color.a*_UseFresnel * lm*2.0;//* lm*8;
				fixed4 finalRGBA = fixed4(col.rgb+fre,1);
				// apply fog
			...
			fixed4 frag_rgb (v2f i) : SV_Target
			{
				...
				half3 fre = pow(1.0-max(0,dot(normalDirection, viewDirection)),_Fresnel)*unity_FogColor.rgb*_Color.a*_UseFresnel;
				fixed4 finalRGBA = fixed4(col.rgb+fre,1);
			...
			fixed4 frag_rgbm (v2f i) : SV_Target
			{
				...
				fixed4 colA = tex2D(_MainTex, i.uv);
				half3 fre = pow(1.0-max(0,dot(normalDirection, viewDirection)),_Fresnel)*unity_FogColor.rgb*_Color.a * _UseFresnel * lm*2;// * lm*2;
				fixed4 finalRGBA = fixed4(col.rgb+fre,1);
				// apply fog
